...
I will use N subjects based on the power analysis.
...
% 0 3 6 9 13 16
ecc_vars=[0,deg2pixel(3), deg2pixel(4),deg2pixel(5),deg2pixel(6),deg2pixel(7)];
signSelect=[-1,1];
...
for tr=1:length(emat)
    ...
    if   emat(tr,1) ==1
        for iter = 1:round(fixLen/ifi)
            %             Screen('DrawLine', wptr,255*[0,1,0], cx-10, cy, cx+10, cy, 3);
            Screen('DrawLine', wptr,255*[0,1,0], round(TargetLocGrid(1)-deg2pixel(1)),...
                round(cy+deg2pixel(0.5)), round(TargetLocGrid(1)+deg2pixel(1)), round(cy+deg2pixel(0.5)), penWidth);
            Screen('Flip',wptr);
        end
    else
        for iter = 1:round(fixLen/ifi)
            %             Screen('DrawLine', wptr,255*[0,1,0], cx-10, cy, cx+10, cy, 3);
            Screen('DrawLine', wptr,255*[0,1,0], cx-(rect(3)/2), round(cy-deg2pixel(0.5)), cx+(rect(3)/2), round(cy-deg2pixel(0.5)), penWidth);
            Screen('DrawLine', wptr,255*[0,1,0], cx-(rect(3)/2), round(cy+deg2pixel(0.5)), cx+(rect(3)/2), round(cy+deg2pixel(0.5)), penWidth);
            Screen('Flip',wptr);
        end
    ...
    elseif emat(tr,2)==2
        for on=1:round(tLength2*ifi)
            Screen('DrawDots', wptr, TargetLocGrid, TargetSize, [255 255 255], [], 2);
            Screen('Flip',wptr);
        end
        
        for gap=1:round(tISI*ifi)
            Screen('FillRect', wptr, [0 0 0]);
            Screen('Flip',wptr);
        end
        
        for off=1: round(tLength2*ifi)
            Screen('DrawDots', wptr, TargetLocGrid, TargetSize, [255 255 255], [], 2);
            Screen('Flip',wptr);
        end
...
%         [keyIsDown, ~, keyCode] = PsychHID('KbCheck');
    ...
    allList=[tr emat(tr,:) response(tr)];
...
catData <- read_csv("sub1_result.csv")
head(catData)

#### Prepare data for analysis - create columns etc.

cleanedData = catData %>% 
  mutate(accuracy = (cue == response)) # mutate to get accuracy


preData<-cleanedData %>% 
  group_by(targetType) %>%
  summarize(AvgAccuracy = mean(accuracy), avgRT=mean(RT)) # the na.rm tells R to ignore NA values
...
```{r,eval=FALSE}
# 3-way Anova
Result <- aov(Y ~ Cuing * Eccentricity * ISI, data = cleanedData)
summary(Result)
```
...
```{r,eval=FALSE}
ggplot(OnlyKeyColumns, aes(x=Cueing, y=dPrime))+
  geom_point()+
