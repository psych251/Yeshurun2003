...
Ths link for the exerpeimental code is posted in the github ![**[link]**]()
...
```{r}
# 3-way Anova
Result <- aov(cleanedData$accuracy ~ cue * targetLoc * ISI, data = cleanedData)
summary(Result)
```
...
```{r}
ggplot(preData, aes(x=targetType, y=AvgAccuracy))+
geom_bar(stat="identity")+
ggtitle("Cueing Condition")
```
```{r}
fd=tibble(cond = "Cued", "Neutral")
cond = c("Cued", "Neutral")
accuracy=c(2.74,2.84)
fd=tibble(cond,temp)
```


```{r}
myplot <- ggplot(fd, aes(x=cond, y=accuracy))+
geom_bar(stat="identity")+
coord_cartesian(ylim=c(1.25, 3.75))+
ggtitle("Cueing Condition") 
```
...
```{r}
df <- data.frame(ISI=c("11", "23", "35"),neutral=c(1.3, 3.4, 3.65), cued=c(1.25,3.22,3.68))

df2 <- data.frame(cond=rep(c("Cued", "Neutral"), each=3),
                ISI=rep(c("11", "23", "35"),2),
                accuracy=c(1.25,3.18,3.5,1.5, 3.4, 3.63))
a=c(1.25,3.18,3.8)
b=c(1.5, 3.4, 3.63)
...
```{r}
myplot <- ggplot(df2, aes(x=ISI, y=accuracy, group=cond)) +
